  ...
  return (
    <div className='fixed top-10 place-self-center z-999 bg-white w-96 h-max-content p-4 border border-gray-200 rounded shadow-xl flex flex-col items-center gap-2 text-center'>
      ...
      <div className='flex justify-center gap-2'>
        <button className={`border border-pandesal-blue text-black bg-pandesal-blue duration-100 text-xs rounded p-2 ${!loading && 'hover:bg-pandesal-blue/70 hover:text-white'}`} disabled={loading ? true : false} onClick={confirm}>{!loading ? 'Confirm' : 'Loading...'}</button>
        <button className={`border-none text-gray-500 duration-100 text-xs rounded p-2 ${!loading && 'hover:bg-pandesal-orange hover:text-white'}`} disabled={loading ? true : false} onClick={cancel}>Cancel</button>
